...
partial class Steps
{
   public sealed class TheWhenSomeFunc
   {
      /* ------------------------------------------------------------ */
      // Given
      /* ------------------------------------------------------------ */
      ...
      public IAssert<TAsserts, TAsserts> was_invoked_once_with<T, TReturn, TAsserts>(T expected)
         ...
         => AtomicAssert<TAsserts, TAsserts>
           .Create($@"{nameof(the_whenSome_Func)}_{nameof(was_invoked_once_with)} ""{expected}""",
                   assert => assert
                            .WhenSome()
      ...
      public IAssert<TAsserts, TAsserts> was_invoked_once_with<T0, T1, TReturn, TAsserts>(T0 expectedArg0,
         ...
         => AtomicAssert<TAsserts, TAsserts>
           .Create($@"{nameof(the_whenSome_Func)}_{nameof(was_invoked_once_with)} ""{expectedArg0}"", ""{expectedArg1}""",
                   assert => assert
                            .WhenSome()
      ...
      public IAssert<TAsserts, TAsserts> was_not_invoked<T, TReturn, TAsserts>()
         where TAsserts : IAsserts, IWhenSomeFuncAsserts<T, TReturn, TAsserts>
         => AtomicAssert<TAsserts, TAsserts>
           .Create($"{nameof(the_whenSome_Func)}_{nameof(was_not_invoked)}",
                   assert => assert
                            .WhenSome()
                            .WasNotInvoked());
