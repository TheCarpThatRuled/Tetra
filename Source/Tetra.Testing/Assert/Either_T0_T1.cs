...
partial class Assert_Extensions
{
      ...
      => actual
        .IsALeft(description,
                 (leftMessage,
                  left) => assert,
                 (rightMessage,
                  right) => throw Failed.Assert(rightMessage,
                                                right.ToTestOutput()),
   ...
   public static Assert IsALeft<TLeft, TRight>(this Assert            assert,
      ...
      => actual
        .IsALeft(description,
                 (leftMessage,
                  left) => assert.AreEqual(leftMessage,
                                           expected,
                                           left.Content),
                 (rightMessage,
                  right) => throw Failed.Assert(rightMessage,
      ...
      => actual
        .IsARight(description,
                  (rightMessage,
                   right) => assert,
                  (leftMessage,
                   left) => throw Failed.Assert(leftMessage),
                  unrecognisedMessage => throw Failed.Assert(unrecognisedMessage));
   ...
   public static Assert IsARight<TLeft, TRight>(this Assert            assert,
      ...
      => actual
        .IsARight(description,
                  (rightMessage,
                   right) => assert.AreEqual(rightMessage,
                                             expected,
                                             right.Content),
                  (leftMessage,
                   left) => throw Failed.Assert(leftMessage),
   ...
   internal static TReturn IsALeft<TLeft, TRight, TReturn>(this IEither<TLeft, TRight>                              actual,
                                                           string                                                   description,
   ...
   internal static TReturn IsARight<TLeft, TRight, TReturn>(this IEither<TLeft, TRight>                              actual,
                                                            string                                                   description,
                                                            Func<string, Either<TLeft, TRight>.RightEither, TReturn> whenRight,
                                                            Func<string, Either<TLeft, TRight>.LeftEither, TReturn>  whenLeft,
                                                            Func<string, TReturn>                                    whenUnrecognised)
   ...
   internal static TReturn Switch<TLeft, TRight, TReturn>(this IEither<TLeft, TRight>                      actual,
